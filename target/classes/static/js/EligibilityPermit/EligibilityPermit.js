    ...
    $.ajax({
        ...
        success:function(data){
            if (data.code == "0") {
                ...
                {  var j=parseInt(i)+1;
                    var imgpath=files[i];
                   var src=get_big_image(imgpath);
                    var $img=$("<div><img src="+imgpath+" alt=\"许可证\" layer-src="+src+" style=\"width: 864px;height: 422px;\"></div>");
                    $(".slider_one_big_picture .next_button").before($img);
                }
                slider_container_init(); //对许可证轮播图的初始化
                layer.ready(function () {
                    layer.photos({
                        photos:".slider_one_big_picture",
                        shift:5
                    });
                })
                $(".slider_one_big_picture").find("img").click(function () {
