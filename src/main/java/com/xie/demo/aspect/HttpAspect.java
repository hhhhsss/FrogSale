...
@Pointcut("execution(public * com.xie.demo.controller.ForgSale.*(..))")
    public void log(){
}
...
@Before("log()")
    ...
    @Around("execution(public * com.xie.demo.controller.ForgSale.into_CustomerService(..))")
    public Object CustomerAroundAdvice(ProceedingJoinPoint proceedingJoinPoint) throws Throwable{
        if(CustomerIsLogin()==true){
            return proceedingJoinPoint.proceed();
        }else{
            throw  new  ForgSaleException(ResultEnum.CustomerSerive_Visit_ERROR);
        }
    }
    ...
    public boolean CustomerIsLogin()
    {
        ...
        if((request.getSession().getAttribute("Customer_username"))!=null){
            username=(String)request.getSession().getAttribute("Customer_username");
        }
        else{
            return false;
        }
        ...
        {
            password=(String)request.getSession().getAttribute("Customer_password");
        }
        else
        {
            return false;
